...
const audioProcessorUrl = "https://flexatar-generic-shared-files.s3.amazonaws.com/js/audio_processor.js"
...
class AnimCalc{
    ...
    getAnim(aBuffer){
        const result = this.makeMel(aBuffer);
        if (result) {
            return result;
        }else{
    ...
    makeMel(aBuffer){
        if (!this.speechNetworks.isReady) {return}
...
class FlexatarUnit{
    constructor(arrayBuffer,flexatarCommon){
        ...
        if (isB64){
            ...
            // console.log("time ",now - window.performance.now())
            uint8Array = bytes
        }
...
class FlexatarClient{
    #token = null
    static route = "https://86s46xzcnk.execute-api.us-east-1.amazonaws.com/default/ftar"
    constructor(token) {
       this.#token = token
...
class Flexatar{
    ...
    constructor(flexatarLink) {
        ...
        const options = {
            ...
            fetch(FlexatarClient.route, options)
                .then(response => {
                    return response.arrayBuffer();
                    
                }).then(flexatarPackage =>{
                    // console.log(flexatarPackage)
...
class SpeechAnimator{
    ...
    constructor(){
        ...
        let myWorker
        if (!noWorklet){
            myWorker = new Worker(nnWorkerUrl);
        ...
        this.processorNode = Promise.all([
            ...
            processorNode.port.onmessage = (event) => {
                ...
                if (data && this.active && data instanceof Float32Array) {
                    if (noWorklet){
                        if (this.onFrame) this.onFrame(this.animCalc.getAnim(event.data))
                    }else{
